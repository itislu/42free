...
prompt_restore()
{
    pretty_print "Do you wish to leave it like that (${sty_bol}l${sty_res}) or restore what was already moved to '$target_path' back to '$source_path' (${sty_bol}r${sty_res})?"
    while true; do
        read -rp "> "
        if [[ $REPLY =~ ^[Rr](estore)?$ ]]; then
            ...
            pretty_print "Invalid option. Please enter ${sty_bol}l${sty_res} for 'leave' or ${sty_bol}r${sty_res} for 'restore'."
        fi
    done
}
...
restore_after_error()
{
    ...
    if [ $ret -ne 0 ]; then
        find "$restore_from" -type d -empty -delete 2>/dev/null
        return 1
    else
        find "$restore_from" -type d -empty -delete 2>/dev/null
        return 0
...
for arg in "${args[@]}"; do
    ...
    if ! stderr=$(rsync -a --remove-source-files "$source_path" "$target_dirpath/" 2>&1); then
        ...
        #TODO Ask user if they wish to see the error messages
        #print_stderr
        syscmd_failed=true

        if [ -d "$source_path" ]; then
            #TODO Possibly still empty directory structure left behind
            # Rename the directory with the files that could not be moved
            timestamp=$(date +%Y%m%d%H%M%S)
            source_old="$source_path~42free-old_$timestamp~"
            if mv "$source_path" "$source_old" 2>/dev/null; then
                ln -s "$target_path" "$source_path" 2>/dev/null
                pretty_print "Symbolic link created and the files that could not be moved are left in $source_old."
            else
                ln -s "$target_path" "$source_path~42free_tmp~" 2>/dev/null
            ...
            if prompt_restore; then
                ...
                if restore_after_error "$target_path" "$source_dirpath/"; then
                    pretty_print "'$source_basename' has been restored to '$source_dirpath'."
                else
                    pretty_print "$print_error Could not fully restore '$source_basename' to '$source_dirpath'."
                    pretty_print "The rest of the partial copy is left in '$target_path'."
                fi
            else
                pretty_print "Try to close all programs and move the rest manually."
            fi
        else
            #TODO Sanity-check this
            first_dir_after_base="$source_base/${arg%%/*}"
            find "$first_dir_after_base" -type d -empty -delete 2>/dev/null
            rmdir "$first_dir_after_base" 2>/dev/null
    ...
    # Remove empty parent directories left in source
    first_dir_after_base="$source_base/${arg%%/*}"
    find "$first_dir_after_base" -type d -empty -delete 2>/dev/null
    rmdir "$first_dir_after_base" 2>/dev/null
    if ! $reverse; then
        # Create the symbolic link
        if stderr=$(ln -s "$target_path" "$source_path" 2>&1); then
            ...
            if stderr=$(ln -s "$target_path" "$source_path~42free_tmp~" 2>&1); then
                pretty_print "Symbolic link left behind with a tmp name."
            else
                print_stderr
            fi
